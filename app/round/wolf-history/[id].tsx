...
import { calculateWolfSettlement, formatWolfPoints } from '@/lib/wolf';
import { getWolfHistorySummary, type WolfGameSummary } from '@/lib/wolfBackend';
import { useAuth } from '@/providers/AuthProvider';
import type { GolfCanadaPostingRecord, SavedRound, WolfScoringMode } from '@/types/round';
...
export default function WolfHistoryDetailScreen() {
    ...
    const load = async () => {
      try {
        ...
        const nextHistoryMeta = findHistoryBackendRowByRouteId(backendHistoryRows, params.id)
          ?? backendHistoryRows.find((row) => row.round_game_id === (round?.backendRoundGameId ?? params.id) && row.game_type === 'wolf')
          ?? null;
        ...
        const [nextSummary, nextRegularDetail] = await Promise.all([
          getWolfHistorySummary(backendRoundGameId),
          backendRoundId && user?.id
            ? getRegularRoundHistoryDetail({ roundId: backendRoundId, roundGameId: backendRoundGameId, gameType: 'wolf', userId: user.id, source: 'detail_screen' }).catch(() => null)
        ...
        setError(nextError?.message ?? 'Wolf history is unavailable.');
      } finally {
  ...
  const handleRetryBackendSave = async () => {
    ...
    try {
      ...
      if (syncResult.cancelled) {
        ...
        const nextSummary = await getWolfHistorySummary(syncResult.round.backendRoundGameId);
        setSummary(nextSummary);
  ...
  const effectiveScoringMode: WolfScoringMode = savedRound?.wolfScoringMode ?? summary?.scoring_mode ?? 'net';
  const roundComplete = summary?.status === 'completed' || (summary?.holes.length ?? 0) >= 18;
  const wolfSettlement = useMemo(() => (
    summary && roundComplete && summary.buy_in_cents > 0 && summary.standings.length === 4
  ...
  return (
    <View style={styles.screen}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        <Text style={styles.title}>Wolf History</Text>
        <Text style={styles.subtitle}>
          {(savedRound?.group?.groupName ?? 'Wolf')} | {savedRound?.date ?? (historyMeta ? historyDateFromBackendRow(historyMeta) : 'Backend round')}
        </Text>
        ...
        {!savedRound && !summary && !loading ? (
          ...
          <SectionCard>
            <Text style={styles.body}>Loading Wolf history...</Text>
          </SectionCard>
        ) : error ? (
          <SectionCard>
            <Text style={styles.emptyTitle}>Wolf history unavailable</Text>
            <Text style={styles.body}>{error}</Text>
          </SectionCard>
        ) : !summary ? (
          <SectionCard>
            <Text style={styles.emptyTitle}>No backend Wolf history found</Text>
            <Text style={styles.body}>This round was saved locally, but no synced Wolf history was returned from the backend.</Text>
            {backendStatusLabel ? (
              <>
          ...
          <>
            ...
            {(savedRound || regularDetail || summary || backendRoundId) ? (
              <GolfCanadaSection
                ...
                prep={golfCanadaPrep}
                description="Post only your own completed golf scorecard from this Wolf round to Golf Canada."
                onPost={() => router.push(
                  savedRound
                    ? (`/round/golf-canada-webview/${savedRound.id}?source=wolf` as any)
                    : (`/round/golf-canada-webview/${roundGameId ?? params.id}?source=wolf` as any),
                )}
            ...
            <SectionCard>
              ...
              <Text style={styles.body}>Wolf players: {summary.active_player_count}</Text>
            ...
            <SectionCard>
              ...
              <WolfSettlementBreakdown
                ...
                emptyText={summary.buy_in_cents <= 0 ? 'No buy-in for this Wolf game' : null}
                ...
                {summary.standings.map((row) => (
                  <View key={row.participant_id} style={styles.playerCard}>
                    <View style={styles.playerHeader}>
                      ...
                      <Text style={styles.playerValue}>{formatWolfPoints(row.total_points)}</Text>
                    </View>
                    <Text style={styles.playerMeta}>Gross {row.gross_total ?? '--'} | Holes {row.holes_complete}</Text>
                    <Text style={styles.playerMeta}>Won {row.holes_won} | Lost {row.holes_lost} | Tied {row.tied_holes}</Text>
                    <Text style={styles.playerMeta}>Lone W {row.lone_wolf_wins} | Lone L {row.lone_wolf_losses}</Text>
                    <Text style={styles.playerMeta}>Blind W {row.blind_wolf_wins} | Blind L {row.blind_wolf_losses}</Text>
                  </View>
                ))}
              </View>
            </SectionCard>
            ...
            <SectionCard>
              ...
              <View style={styles.cardList}>
                {summary.wolf_order_participant_ids.map((participantId, index) => {
                  const participant = summary.standings.find((row) => row.participant_id === participantId);
                  return (
                    <Text key={`wolf-order-${participantId}`} style={styles.body}>
                      {index + 1}. {participant?.display_name ?? 'Player'}
                    </Text>
                  );
                })}
              </View>
                ...
                {summary.holes.map((hole) => (
                  <View key={`wolf-hole-${hole.hole_number}`} style={styles.holeCard}>
                    ...
                    <Text style={styles.playerMeta}>
                      {hole.is_blind_wolf
                        ? `${hole.wolf_display_name ?? 'Wolf'} blind`
                        : hole.is_lone_wolf
                          ? `${hole.wolf_display_name ?? 'Wolf'} alone`
                          : `${hole.wolf_display_name ?? 'Wolf'} and ${hole.partner_display_name ?? 'Partner'}`}
                    </Text>
