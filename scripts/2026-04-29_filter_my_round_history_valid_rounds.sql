...
score_rollup as (
  select
    ...
    count(distinct case when hs.user_id = me.user_id and hs.strokes is not null and hs.strokes > 0 then hs.hole_number end)::int as holes_complete,
...
eligible_rounds as (
  select
    ...
    coalesce(sr.current_user_score, 0) as current_user_score,
    coalesce(sr.holes_complete, 0) as holes_complete,
    coalesce(sr.hole_score_row_count, 0) as hole_score_row_count
  from public.rounds r
  cross join me
  join score_rollup sr
    on sr.round_id = r.id
  ...
  where me.user_id is not null
    ...
    and coalesce(sr.current_user_score, 0) > 0
    and coalesce(sr.holes_complete, 0) > 0
    and coalesce(sr.hole_score_row_count, 0) > 0
    and lower(coalesce(r.status, '')) not in ('deleted', 'cancelled', 'canceled', 'abandoned')
),
selected_games as (
  select
    er.id as round_id,
  ...
  left join lateral (
    ...
    where rg.round_id = er.id
      and lower(coalesce(rg.status, '')) not in ('deleted', 'cancelled', 'canceled', 'abandoned')
    order by
      case
...
select *
from history_rows
where coalesce(current_user_score, 0) > 0
  and coalesce(holes_complete, 0) > 0
  and coalesce(hole_score_row_count, holes_complete, 0) > 0
  and (
    lower(coalesce(status, '')) <> 'draft'
    or coalesce(holes_complete, 0) >= 18
  )
